...
export default function FavoriteBooks() {
  ...
  const booksIsFavorite = bookshelf.filter((book) => book.isFavorited);
  ...
  return (
    <section className="flex flex-col justify-center items-center min-h-full px-8 pb-14 md:px-10">
      <TitleContent text="Buku Favorit" brand="RakBukuKu" />
      {booksIsFavorite.length < 1 && (<BooksEmptyMessage message="Data Buku Favorit Kosong" />)}
      {booksIsFavorite.length > 0
        && (
        <CardBookContainer>
          {booksIsFavorite.map((book) => (
